...
### تست Chain Proxy قبل از اعمال

یک دکمه **Test** کنار فیلد Chain Proxy هست. این تست پروکسی رو **از سمت خود Worker** (از لبه‌ی Cloudflare، نه از دستگاه شما) بررسی می‌کنه؛ بنابراین می‌تونید قبل از اعمال و بروزرسانی اشتراک مطمئن بشید که پروکسی هنوز زنده‌ست:

1. کانفیگ Chain Proxy رو توی فیلد وارد کنید.
2. روی **Test** بزنید — نیازی به Apply نیست.
3. نتیجه رو بخونید: **Working** (سبز)، **Reachable** (نارنجی) یا **Unreachable** (قرمز)، همراه با زمان پاسخ اندازه‌گیری‌شده.

تست دقیقاً چی رو بررسی می‌کنه به پروتکل بستگی داره:

* **Socks و Http** — یک تونل واقعی از طریق پروکسی به یک هدف ثابت Cloudflare باز می‌شه، پس نتیجه ثابت می‌کنه که پروکسی می‌تونه ترافیک رو سرتاسری عبور بده.
* **VLESS، Trojan، Shadowsocks و VMess** — پنل به سرور پروکسی از طریق TCP دسترسی پیدا می‌کنه و نتیجه رو *Reachable* نشون می‌ده، ولی اشاره می‌کنه که عبور رمزنگاری‌شده از پنل قابل تأیید نیست (این دست‌ها فقط سمت کلاینت انجام می‌شه). برای تضمین نهایی یک بار Apply کنید و در کلاینت خودتون بررسی کنید.

!!! note "یادداشت"
    این تست اندازه می‌گیره که آیا پنل (Cloudflare) می‌تونه به VPS پشت پروکسی دسترسی داشته باشه. این یک بررسی دسترسی‌پذیری است، نه شبیه‌سازی کامل کلاینت.
...
هر ورودی یک نام اختیاری بعد از `#` می‌تونه داشته باشه — مثل `143.112.242.123#Fast-Direct`، `104.16.1.1:443#Cloudflare-DE` یا فقط `143.112.242.123`. این نام جای نوع عمومی توی اسم کانفیگ می‌شینه (مثل `💦 1. VLESS - Fast-Direct : 443`) و به‌صورت توکن `{IPNAME}` توی [قالب اسم کانفیگ](config-names.md) هم در دسترسه. ورودی‌های بدون `#` مثل قبل اسم پیش‌فرض می‌گیرن.
...
میتونید یک یا چند NAT64 Prefix [از اینجا](https://github.com/Nexuspt753/BPB-Worker-Panel/blob/main/docs/NAT64Prefixes.md) انتخاب کنید که IPها رو بر اساس منطقه و ISP لیست کرده.
